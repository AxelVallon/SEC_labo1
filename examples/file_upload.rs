...
lazy_static! {
    static ref HASHMAP: Mutex<HashMap<String, MediaFile>> = Mutex::new(HashMap::new()); // str faster than String for search and static
}
...
fn upload_file(uuid: &String, filepath: &String, media_type: String) -> bool {
    ...
    match map.get(uuid) {
        ...
        None => {
            map.insert(uuid.clone(), MediaFile { path: filepath.clone(), media_type: media_type.clone() });
            true
        }
    }
}
...
fn file_upload_handler() {
    loop {
        let filepath = input::<String>().msg("Please enter the path to an image or video file : ").get();
        match validate_file(&filepath, true) { 
            Ok(media_type_opt) => { //The selected File exist
                match media_type_opt {
                    Some(media_type) => { // The selected file is valid and we retrieve his media type ("video" or "image")
                        ...
                        let uuid = Uuid::new_v5(&Uuid::default(), buffer).as_hyphenated().to_string();
...
// - Test the exitence in Hashmap of the file 
// - We verify if the file still exist in the fs and if the content is the same. 
fn file_verify_handler() {
    loop {
        let uuid_input = input::<String>().msg("Please enter the UUID to check : ").get();
        if !validate_uuid(&uuid_input) {
            println!("The provided UUID is not valid")
        }
        else {
            match retrieve_with_uuid(&uuid_input) { // we verify if it's saved
                Some(media_file) => {
                    match validate_file_with_uuid(&uuid_input, &media_file.path) { // we verify if the content has been modified
                        Ok(identical_file) => { 
                            if identical_file {
                                println!("File {} exists, it is a/an {} file", uuid_input, media_file.media_type);
                                break;
                            }
                            else{ //TODO : Remove this if this leak info on your infra
                                println!("The file has been modified, the content is not the same");
                            }
                        } //TODO : Remove this if this leak info on your infra
...
fn get_url_handler() {
    ...
    } else { // we only give the File not found info to user there. More info could leak something.
        println!("File not found. Please refer to option 2 : \"Verify file exists\"")
    }
}
